...
describe ActiveFacets::Serializer::Base do
  ...
  describe "#config" do
    ...
    it { expect(subject.class).to be(ActiveFacets::Config)}
  ...
  describe "#expose" do
    ...
    context "all" do
      ...
      it { expect{subject}.to raise_error(ActiveFacets::Errors::ConfigurationError,ActiveFacets::Errors::ConfigurationError::ALL_FIELDS_ERROR_MSG) }
    end
    context "all_attributes" do
      let(:field_set_name) { :all_attributes }
      it { expect{subject}.to raise_error(ActiveFacets::Errors::ConfigurationError,ActiveFacets::Errors::ConfigurationError::ALL_ATTRIBUTES_ERROR_MSG) }
    end
  end
  ...
  describe "instance methods" do
    ...
    describe ".from_hash" do
      before do
        allow_any_instance_of(ActiveFacets::Serializer::Facade).to receive(:from_hash) { |attributes| attributes }
      end
      subject { instance.from_hash(resource, { foo: :bar }) }
      it { expect(subject).to eq({ foo: :bar }) }
      it { expect(ActiveFacets::Serializer::Facade).to receive(:new).once }
    end

    describe ".as_json" do
      before do
        allow_any_instance_of(ActiveFacets::Serializer::Facade).to receive(:as_json) { { foo: :bar } }
      end
      ...
      context 'collection' do
        ...
        it { expect(ActiveFacets::Serializer::Facade).to receive(:new).twice }
      end
    end
    ...
    describe ".get_custom_serializer_class" do
      ...
      context "unregistered" do
        ...
        skip { expect{subject}.to raise_error(ActiveFacets::Errors::LookupError, 'Unable to locate serializer for:: ["ResourceA", "Unregistered", :attribute_serializer, 1.0, nil]') }
      end
    end
