...
describe ActiveFacets::Serializer::Facade do
  ...
  let(:options) { { ActiveFacets.opts_key => opts } }
  let(:opts) {
    {
      ActiveFacets.fields_key => fields,
      ActiveFacets.field_overrides_key => field_overrides,
      ActiveFacets.version_key => version,
      ActiveFacets.filters_key => filters
    }
  }
  let(:fields) { :fields }
  ...
  describe ".as_json" do
    ...
    before do
      allow(ActiveFacets.document_cache).to receive(:fetch).and_call_original
      allow(instance).to receive(:serialize!).and_call_original
      subject
    end

    it { expect(ActiveFacets.document_cache).to have_received(:fetch) }
  ...
  describe ".is_expression_scopeable?" do
    ...
    let(:persisted) { true }
    let(:options) { { ActiveFacets.opts_key => opts.merge(ActiveFacets.filters_force_key => filters_enabled_locally ) } }
  ...
  describe ".is_relation_scopeable?" do
    ...
    let(:exp) { ActiveRecord::Relation.new(nil,nil) }
    around do |example|
      temp = ActiveFacets::filters_enabled
      ActiveFacets::filters_enabled = filters_enabled_globally
      example.run
      ActiveFacets::filters_enabled = temp
    end
    ...
    context "globally disabled" do
      ...
      context "forced disabled" do
        let(:options) { { ActiveFacets.opts_key => opts.merge(ActiveFacets.filters_force_key => false ) } }
        it { expect(subject).to be false }
      end
      context "forced enabled" do
        let(:options) { { ActiveFacets.opts_key => opts.merge(ActiveFacets.filters_force_key => true ) } }
        it { expect(subject).to be true }
      end
    end
  ...
  describe ".get_resource_attribute!" do
    ...
    let(:field) { :field }
    context "missing" do
      it { expect{subject}.to raise_error(ActiveFacets::Errors::AttributeError) }
    end
